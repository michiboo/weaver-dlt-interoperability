...
const hashfunc = require("./hashFunction.js");
...
async function claimToken(interopContract, lockContractId, recipient, preimage) {
	...
	const hashedPreimage = new hashfunc.SHA256()
	hashedPreimage.setPreimage(preimage)
	const protobuf = createAssetClaimInfoSerialized(hashedPreimage)
	console.log("\n lockcontractid: %s, recipient: %s, claiminfo: %o", lockContractId, recipient, protobuf)
	var claimStatus = await interopContract.claimAsset(lockContractId, protobuf, {
		from: recipient
	}).catch(function (err) {
		console.log(err)
...
async function lockToken(sender, recipient, tokenContract, tokenAmount, interopContract, hashLock, timeLock, tokenId = 0, data = "", web3N) {
	...
	const encodedRecipient = web3N1.eth.abi.encodeParameter('address', recipient)
	const protobufParams = createAssetExchangeAgreementSerialized("1", tokenId, recipient.slice(2), "")
	...
	// await interopContract.returnProtobuf(protobufParams, { from: sender })
...
async function main() {
	...
	let lockTx1 = await lockToken(Alice1, Bob1, AliceERC20, tokenAmount, interopContract1, hash, timeLockSeconds, tokenId)
	if (!lockTx1) {
		console.log("\n !!! Locking of Alice's tokens failed in Netowrk 1. Aborting here !!!")
		return
	}
	let lockContractId1 = lockTx1.logs[0].args.lockContractId
	console.log("Reciever 1: ", lockTx1.logs[0].args.receiver)
	...
	console.log("Reciever 2: ", lockTx2.logs[0].args.sender) // 
